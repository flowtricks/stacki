...
function writeMarkerConfig(projectPath) {
  try {
    ...
    const projectDir = toPosix(projectPath);
...
const avbMarkers = {
  ...
  load(id) {
    ...
    // Every .astro the project owns, not just pages: a component's own markup
    // has to carry markers for its internals to be selectable once the editor
    // is drilled into it. Files that don't round-trip through the model (the
    // \`editable\` gate below) are served untouched, exactly as before — they
    // just aren't addressable on the canvas.
    ...
    try {
      ...
      // Project-relative posix path — the file half of every node key this
      // file emits. The app derives the same string from its open file's
      // absolute path, so the two agree without a shared registry.
      return serializePageMarked(parsed.model, file.slice(PROJECT_DIR.length + 1));
    } catch {
      return null;
    }
  },
