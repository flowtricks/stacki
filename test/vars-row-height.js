  ...
  fs.writeFileSync(
    ...
    `const { app, BrowserWindow } = require('electron');
     ...
     app.whenReady().then(async () => {
       ...
       out.menuRows = await js("(() => { const menu = document.querySelector('.vars-menu'); if (!menu) return null; const items = [...menu.querySelectorAll('.vars-menu-item')]; const m = menu.getBoundingClientRect(); return { labels: items.map((i) => i.textContent.trim()), justify: getComputedStyle(items[0]).justifyContent, iconLefts: items.map((i) => Math.round(i.querySelector('svg').getBoundingClientRect().left - m.left)), sameStart: new Set(items.map((i) => Math.round(i.querySelector('svg').getBoundingClientRect().left))).size === 1 } })()");
